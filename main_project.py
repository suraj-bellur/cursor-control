...
def angle(point1):
	slope12 = (point1[1] - 250)/(point1[0] - 250)
...
while(time.time() - currenttime <= 25): #The calibration code will run for 25 seconds.
	ret,image = cap.read()
	...
	for (i,rect) in enumerate(rects): # Loop used for the prediction of facial landmarks 
		...
		cv2.drawContours(image,[mouthroi],-1,(255,0,0),2)
		...
		if elapsedTime < 5.0: # recording the phase where both eyes are open
			...
			cv2.putText(blackimage,'Blink Left Eye',(0,100), font, 1,(255,255,255),2,cv2.LINE_AA)
			...
			cv2.putText(blackimage,'Blink Right Eye',(0,100), font, 1,(255,255,255),2,cv2.LINE_AA)
			...
			cv2.putText(blackimage,'Open and close Your Mouth',(0,100),font,1,(255,255,255),2,cv2.LINE_AA)
...
while(True):
	try: 
		...
		# Counting number of non zero pixels in the thresholded images
			...
			cv2.line(image,(250,250),(h,k),(255,255,255),2) # Draws a line between the tip of the nose and the centre of reference circle
		...
		cv2.circle(image,(h,k),2,(0,255,0),-1)
